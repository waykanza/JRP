...
function loadData3() {
	if (popup) { popup.close(); }
	var data = jQuery('#form').serialize();
	jQuery('#data-load').load(this_base + 'reserve_persediaan_load.php', data);
	return false;
}
...
function showPopup(act, id) {
	var url = base_marketing_operasional + 'persediaan_awal/reserve.php?act=' + act + '&id=' + id;
	setPopup('Reserve', url, 500, 300);
	return false;
}

function showPopup2(act, id) {
	var url = this_base + 'informasi_persediaan_popup.php?act=' + act + '&id=' + id;
...
{
	var url = this_base + 'spp_popup.php' + '?act=' + act + '&id=' + id + '&nm=' + nm;
