...
<table class="t-control2">
    <tr>
      <td style="font-size:20px;text-align:center;font-weight:bold" >Daftar Surat Pemberitahuan Pengurusan Akad </td>
    </tr>
</table>	
...
jQuery(function($) {
	...
	$(document).on('click', '#surat', function(e) {
		...
		if (checked < 1) {
			...
			e.preventDefault();
			location.href = base_marketing + 'collection_tunai/surat/spk/pemberitahuan_akad/surat_pemberitahuan_akad.php?' + $('#form').serialize();	
			//cetakSurat();
		}
		return false;
	});
	...
	$(document).on('click', '#print', function(e) {
		e.preventDefault();
		location.href = base_marketing + 'collection_tunai/surat/spk/pemberitahuan_akad/print_pemberitahuan_akad.php?' + $('#form').serializeArray();
		return false;
	});
...
function loadData()
{
	...
	jQuery('#t-detail').load(base_marketing + 'collection_tunai/surat/spk/pemberitahuan_akad/pemberitahuan_akad_load.php', data);
	return false;
}

function cetakSurat()
{	
	var url		= base_marketing + 'collection_tunai/surat/spk/pemberitahuan_akad/surat_pemberitahuan_akad.php?act=Surat',
