<?php
	...
	if($act == 'Surat') #Proses Hapus
	{
		...
		{		
			...
			$query = "select NOMOR_SURAT_TEGURAN_3,REG_SURAT_TEGURAN_3 from CS_REGISTER_CUSTOMER_SERVICE";
			$obj = $conn->execute($query);
			
			$no				= 1 + $obj->fields['NOMOR_SURAT_TEGURAN_3'];
			$reg			= $obj->fields['REG_SURAT_TEGURAN_3'];
			$tahun 			= date('Y');
