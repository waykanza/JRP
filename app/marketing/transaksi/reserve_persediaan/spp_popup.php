...
jQuery(function($) {
	...
	$(document).on('click', '#save', function(e) {
		e.preventDefault();
		var url = base_marketing_transaksi + 'reserve_persediaan/spp_proses.php',
			data = $('#form').serialize();
		...
		$.post(url, data, function(data) {
			if (data.error == true)
			{
				...
				else if (data.act == 'Tambah')
				{
					alert(data.msg);
					//parent.loadData();
					parent.showPopup2('Ubah', '<?php echo $id; ?>');
				}
...
function loadData1()
{
	...
	jQuery('#tab1').load(base_marketing_transaksi + 'reserve_persediaan/spp_spp.php', data);	
	return false;
}
...
function loadData2()
{
	if (popup) { popup.close(); }
	var data = jQuery('#form').serialize();
	jQuery('#tab2').load(base_marketing_transaksi + 'reserve_persediaan/spp_rincian_harga.php', data);	
	return false;
}
...
function loadData3()
{
	if (popup) { popup.close(); }
	var data = jQuery('#form').serialize();
	jQuery('#tab3').load(base_marketing_transaksi + 'reserve_persediaan/spp_rencana.php', data);	
	return false;
}
</script>
</head>
