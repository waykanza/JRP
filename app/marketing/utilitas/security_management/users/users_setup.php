...
function loadData() {
	...
	jQuery('#data-load').load(this_base + 'users_load.php', data);
	return false;
}

function showPopup(act, id) {
	var url = this_base + 'users_popup.php?act=' + act + '&id=' + id;
	setPopup(act + ' User', url, 500, 300);
	return false;
}

function hapusData() {
	var url		= this_base + 'users_proses.php?act=Hapus',
...
<div class="title-page">SECURITY MANAGEMENT (USERS)</div>
