...
FROM 
	RENCANA 
WHERE
	(select dbo.kurang_tgl(TANGGAL,$query_pemb_jt)) = CONVERT(DATETIME,'$tgl',105) AND 
	KODE_BLOK NOT IN($query_blok_lunas)
";
$total_data = $conn->execute($query)->fields['TOTAL'];
$total_page = ceil($total_data/$per_page);

$set_jrp = '
<tr><td colspan="8" class="nb text-center"><b> LAPORAN DAFTAR SURAT PEMBERITAHUAN JATUH TEMPO </b></td></tr>
...
<head>
<meta charset="UTF-8">
<title>LAPORAN DAFTAR SURAT PEMBERITAHUAN JATUH TEMPO</title>
<style type="text/css">
@media print {
	@page {
...
if ($total_data > 0)
{
	...
	WHERE
		(select dbo.kurang_tgl(TANGGAL,$query_pemb_jt)) = CONVERT(DATETIME,'$tgl',105)AND 
	B.KODE_BLOK NOT IN($query_blok_lunas)
