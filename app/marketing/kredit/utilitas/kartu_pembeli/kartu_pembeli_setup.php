<div class="title-page">CETAK KARTU PEMBELI</div>
...
function loadData()
{
	...
	jQuery('#t-detail').load(base_marketing + 'kredit/utilitas/kartu_pembeli/kartu_pembeli_load.php', data);	
	return false;
}

function showPopup(act, id)
{
	var url =	base_marketing + 'kredit/utilitas/kartu_pembeli/kartu_pembeli_popup.php' + '?act=' + act + '&id=' + id;	
	setPopup('Cetak Kartu Pembeli', url, 400, 300);	
	return false;
}

function showPopup2(act, id)
{
	var url =	base_marketing + 'kredit/utilitas/kartu_pembeli/bank_popup.php' + '?act=' + act + '&id=' + id;
	setPopup('Edit Bank dan Nomor Rekening', url, 450, 200);
	return false;
}
</script>
