...
<table class="t-control2">
    <tr>
      <td style="font-size:20px;text-align:center;font-weight:bold" >Daftar Surat Pembatalan Penolakkan Kredit </td>
    </tr>
</table>	
...
jQuery(function($) {
	...
	$(document).on('click', '#surat', function(e) {
		...
		if (checked < 1) {
			...
			e.preventDefault();
			location.href = base_marketing + 'collection_tunai/surat/spk/pembatalan_tolak_kredit/surat_pembatalan_tolak_kredit.php?' + $('#form').serialize();	
			//cetakSurat();
		}
		return false;
	});
	...
	$(document).on('click', '#print', function(e) {
		e.preventDefault();
		location.href = base_marketing + 'collection_tunai/surat/spk/pembatalan_tolak_kredit/print_pembatalan_tolak_kredit.php?' + $('#form').serializeArray();
		return false;
	});
...
function loadData()
{
	...
	jQuery('#t-detail').load(base_marketing + 'collection_tunai/surat/spk/pembatalan_tolak_kredit/pembatalan_tolak_kredit_load.php', data);
	return false;
}

function cetakSurat()
{	
	var url		= base_marketing + 'collection_tunai/surat/spk/pembatalan_tolak_kredit/surat_pembatalan_tolak_kredit.php?act=Surat',
